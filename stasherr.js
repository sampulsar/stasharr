...
(function () {
  ...
  let config = {
    ...
    rootFolderPath: "*****", // Path to where you keep your stuff
    searchForMovie: true,
  };
  config.apiUrl = `${config.scheme}://${config.userDomain}/api/v3/`;
  ...
  function handleSceneLookup(button, sceneID, isHeader) {
    if (sceneID) {
      const fullApiUrl = `${config.apiUrl}movie?stashId=${encodeURIComponent(sceneID)}`;
          ...
          if (data?.length > 0) {
            if (data[0].hasFile) {
              ...
              button.addEventListener("click", () =>
                searchSceneInWhisparr(sceneID, button, isHeader)
              );
            }
          } else {
            if (isHeader) {
  ...
  function updateButtonForDownloadedScene(button, isHeader) {
    button.disabled = true;
    button.style.color = "#ffffffcc";
    if (isHeader) {
      button.innerHTML =
        '<i class="fa-solid fa-check-circle"></i> Already Downloaded';
    } else {
  ...
  function createPayload(sceneData) {
    return {
      ...
      monitored: true,
      addOptions: { searchForMovie: config.searchForMovie },
